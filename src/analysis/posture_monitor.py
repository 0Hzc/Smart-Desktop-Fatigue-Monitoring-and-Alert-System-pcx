...
class PostureMonitor:
    ...
    def __init__(
        ...
        # OpenCV坐标系：X向右，Y向下，Z向后（深度为负）
        # 以鼻尖为原点，X坐标反转以匹配摄像头视角
        self.model_points = np.array([
            (0.0, 0.0, 0.0),           # 鼻尖（最突出点，原点）
            (0.0, 330.0, -65.0),       # 下巴（在鼻尖下方+后方）
            (225.0, -170.0, -135.0),   # 左眼外角（在鼻尖上方+后方+右侧）
            (-225.0, -170.0, -135.0),  # 右眼外角（在鼻尖上方+后方+左侧）
            (150.0, 150.0, -125.0),    # 左嘴角（在鼻尖下方+后方+右侧）
            (-150.0, 150.0, -125.0)    # 右嘴角（在鼻尖下方+后方+左侧）
        ], dtype=np.float64)
