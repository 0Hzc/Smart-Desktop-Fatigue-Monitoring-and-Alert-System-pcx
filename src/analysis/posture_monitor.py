...
class PostureMonitor:
    ...
    def __init__(
        ...
        pitch_threshold_down: float = 10.0,
        pitch_threshold_up: float = -4.0,
        ...
        # OpenCV坐标系：X向右，Y向下，Z向前（离开摄像头）
        # 以鼻尖为原点，绕Y轴旋转180°以匹配实际朝向
        self.model_points = np.array([
            (0.0, 0.0, 0.0),           # 鼻尖（最突出点，原点）
            (0.0, 330.0, 65.0),        # 下巴（在鼻尖下方+前方）
            (225.0, -170.0, 135.0),    # 左眼外角（在鼻尖上方+前方+右侧，因旋转180°）
            (-225.0, -170.0, 135.0),   # 右眼外角（在鼻尖上方+前方+左侧，因旋转180°）
            (150.0, 150.0, 125.0),     # 左嘴角（在鼻尖下方+前方+右侧，因旋转180°）
            (-150.0, 150.0, 125.0)     # 右嘴角（在鼻尖下方+前方+左侧，因旋转180°）
        ], dtype=np.float64)
